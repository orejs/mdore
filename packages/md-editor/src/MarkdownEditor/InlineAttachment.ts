...
class FileUpload {
  ...
  constructor(settings: AttachmentSettings, view: EditorView) {
    this.view = view;
  ...
  onFileInserted = (file: File) => {
    if (this.settings.onFileReceived.call(this, file) !== false) {
      ...
      this.lastValue = this.settings.progressText.replace(
        this.filenameTag,
        this.filename,
      );
  ...
  onFileUploadResponse = (xhr: XMLHttpRequest) => {
    if (
      this.settings.onFileUploadResponse.call(this, this.file!, xhr) !== false
    ) {
